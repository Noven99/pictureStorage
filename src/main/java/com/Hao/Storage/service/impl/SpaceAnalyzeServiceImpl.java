...
import com.Hao.Storage.service.SpaceAnalyzeService;
...
public class SpaceAnalyzeServiceImpl extends ServiceImpl<SpaceMapper, Space>
    ...
    public SpaceUsageAnalyzeResponse getSpaceUsageAnalyze(SpaceUsageAnalyzeRequest spaceUsageAnalyzeRequest, User loginUser) {
        ...
        if (spaceUsageAnalyzeRequest.isQueryAll() || spaceUsageAnalyzeRequest.isQueryPublic()) {
            ...
            //补充查询范围（让返回值更加灵活，而不是整个Picture对象）（本地求和，查询大量数据的优化方法）
    ...
    public List<SpaceSizeAnalyzeResponse> getSpaceSizeAnalyze(SpaceSizeAnalyzeRequest spaceSizeAnalyzeRequest, User loginUser) {
        ...
        sizeRanges.put("100KB-1MB", picSizes.stream().filter(size -> size >= 100 * 1024 && size < 1 * 1024 * 1024).count());
        sizeRanges.put("1MB-2MB", picSizes.stream().filter(size -> size >= 1 * 1024 * 1024 && size < 2 * 1024 * 1024).count());
        sizeRanges.put(">2MB", picSizes.stream().filter(size -> size >= 2 * 1024 * 1024).count());
